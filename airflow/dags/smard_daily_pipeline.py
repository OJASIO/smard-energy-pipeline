...
DBT_CMD = DBT + " run --project-dir " + BASE + "/dbt/smard_pipeline --profiles-dir ~/.dbt --target prod"
...
with DAG(
    ...
    dbt_silver = BashOperator(
        ...
        bash_command=ENV + " && " + DBT_CMD + " --select silver",
    )

    dbt_gold = BashOperator(
        task_id="dbt_gold_models",
        bash_command=ENV + " && " + DBT_CMD + " --select gold",
    )

    dbt_test = BashOperator(
        task_id="dbt_test",
        bash_command=ENV + " && " + DBT + " test --project-dir " + BASE + "/dbt/smard_pipeline --profiles-dir ~/.dbt --target prod",
    )

    [ingest_ecb, ingest_eurostat] >> ge_validate >> dbt_silver >> dbt_gold >> dbt_test
