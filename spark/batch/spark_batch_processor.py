...
Reads historical RAW data from BigQuery Bronze
Applies full PySpark cleaning
Writes CLEAN data to Snowflake Silver
Run after smard_historical_ingestion.py
"""
...
def clean_energy_batch(spark, pdf_raw):
    if pdf_raw.empty:
    ...
    df = df.withColumn("data_source", F.lit("historical"))

    # Safe types for Snowflake
    ...
    log(f"Cleaned: {clean_count} rows ({rejected} rejected)")
